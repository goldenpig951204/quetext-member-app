...
const searchOverviewModel = require("../models/searchOverview");
...
const quetextMiddleware = (prefix) => createProxyMiddleware({
    target: `https://www.quetext.com`,
    selfHandleResponse: true,
    changeOrigin: true,
    onProxyReq: (proxyReq, req) => {
        let userAgent = req.headers["user-agent"];
    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (req.path == "/do-auto-login") {
                try {
                    let { username, password } = await credentialModel.findOne({"type": "quetext"});
                    let result = await quetextAutoLogin(username, password);
                    if (result) {
                        res.statusCode = 200;
                        return JSON.stringify({status: true});
                    } else {
            ...
            } else if (req.path == "/limit-error" || req.path == "/error") {
                proxyRes.statusCode = 200;
                res.statusCode =200;
                let html = fs.readFileSync(__dirname + "/../views/warning.ejs");
                return html.toString().replace(/<%= msg %>/g, "Daily Limit Reached.");
            }

            if (proxyRes.headers["location"]) {
                if (req.path == "/login" && proxyRes.headers["location"] == "/search") {
                    proxyRes.statusCode = 200;
                    res.statusCode = 200;
                } else {
                    proxyRes.headers["location"] = proxyRes.headers["location"].replace("https://www.quetext.com", domain);
                    res.setHeader("location", proxyRes.headers["location"].replace("https://www.quetext.com", domain));
                }
            }
            if (
                (typeof req.user == "object" && 
                !req.user.isAdmin && 
                /\/account\/subscription/.test(req.path) ||
                /\/account\/billing/.test(req.path) ||
                /\/account\/users/.test(req.path)) ||
                /^\/$/.test(req.path)
            ) {
                proxyRes.statusCode = 301;
                proxyRes.headers["location"] = domain + "/search";
                res.statusCode = 301;
                res.setHeader("location", domain + "/search");
            }
            if (
                proxyRes.headers["content-type"] && 
                proxyRes.headers["content-type"].includes("text/html")
                ...
                if (
                    typeof req.user == "object" &&
                    !req.user.isAdmin &&
                    /\/account\/settings/.test(req.path)
                ) {  
                    $("#account-subnav + nav").remove();
                    $("#account-subnav").remove();
                    $("#account-section-details").css({
                        "margin-left": "70px"
                    });
                ...
                if (req.path == "/login") {
                    let html = fs.readFileSync(__dirname + "/../views/quetext-auth.ejs");
                    return html.toString();
                } 
                return $.html();
            }
...
const quetextLimitMiddleware = async (req, res, next) => {
    if (!req.originalUrl.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
        let { id, username, isAdmin } = req.user;
        let wpSite = req.wpSite;
        if (
            !isAdmin &&
            /\/text/.test(req.originalUrl)
        ) {
            const total = await searchOverviewModel.countRequests(id, username, wpSite, "quetext");
            const limit = await settingModel.getOverviewLimit("quetextSearchLimit");
            if (total > limit) {
                return {
                    next: false,
                    redirect: false,
                ...
                await searchOverviewModel.create({
                    userId: id,
                    username: username,
                    site: wpSite,
                    proxyType: "quetext",
...
const applyMiddleware = async (req, res, next) => {
    ...
    if (proxy !== null) {
        if (setting != null) {
            ...
            } else if (proxy.type == "quetext") {
                let result = await quetextLimitMiddleware(req, res);
                if (result.next) {
                    return quetextMiddleware(prefix)(req, res, next);
                } else {
